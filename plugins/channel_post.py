...
async def channel_post(client: Client, message: Message):
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("1 Share URL", url=f'https://telegram.me/share/url?url={link}')]])
...
async def new_post(client: Client, message: Message):
    ...
    base64_string = await encode(string)
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("2 Share URL", url=f'https://telegram.me/share/url?url={link}')]])
    try:
